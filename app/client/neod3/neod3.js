...
neo.graphView = (function() {
  function graphView(element, graph, style) {
    ...
    neo.tooltip = d3.select('body').append('div').attr('id', 'tooltip').style('opacity', 0);
  }
...
neo.viz = function(el, measureSize, graph, style) {
  ...
  onNodeMouseOver = function(node) {
    ...
    neo.tooltip.transition().duration(200).style('opacity', .9);
    neo.tooltip.html(text).
  ...
  onNodeMouseOut = function(node) {
    neo.tooltip.transition().duration(500).style('opacity', 0);
