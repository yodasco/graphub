...
neo.viz = function(el, measureSize, graph, style) {
  ...
  viz.fillColorByLanguage = function(node) {
    return neo.fillColosrByLanguage(node.propertyMap.language);
  };
...
neo.fillColosrByLanguage = function(language) {
  let map = {
    'Mercury': '#abcdef',
    'TypeScript': '#31859c',
    'PureBasic': '#5a6986',
  ...
  if (language) {
    let val = map[language];
    if (val) {
      return hexToRgbWithOpacity(val, 0.2);
    }
  }
};


let hexToRgbWithOpacity = function(hex, opacity) {
  ...
  nodeOutline = new neo.Renderer({
    onGraphChange: function(selection, viz) {
      ...
      circles.attr({
        ...
        fill: function(node) {
          return viz.fillColorByLanguage(node) || viz.style.forNode(node).get('color');
        },
